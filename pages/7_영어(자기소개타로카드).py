...
if password == correct_password:
    ...
    if generate_button:
        ...
        if not all([name, likes_items, likes_people, aspiration, want_to_do, favorite_food, favorite_country]):
            ...
            # 이미지 생성 프롬프트를 생성합니다.
            prompt = f"타로 카드 디자인으로 중앙에는 {gender}아이의 일러스트레이션이 있고, 주변에는 그 아이가 좋아하는 물건({likes_items}), 좋아하는 사람({likes_people}), 장래희망({aspiration}), 지금 당장하고 싶은 것({want_to_do}), 좋아하는 음식({favorite_food}), 좋아하는 나라({favorite_country}) 등이 둘러싸고 있습니다. 카드 하단에는 학생의 이름 '{name}'이 크게 표시됩니다."
